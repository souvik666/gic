...
while read -r line; do
  ...
  case $status in
    M) commit_message_lines+=("${GREEN}$MODIFIED_TAG: $file${RESET}");;
    A) commit_message_lines+=("${CYAN}$ADDED_TAG: $file${RESET}");;
    D) commit_message_lines+=("${RED}$DELETED_TAG: $file${RESET}");;
...
if [ -n "$issue_number" ]; then
  commit_message="(#$issue_number) [$date_time] ${commit_message_lines[0]}"
else
  commit_message="[$date_time] ${commit_message_lines[0]}"
fi

for ((i=1; i<${#commit_message_lines[@]}; i++)); do
  commit_message+="\n${commit_message_lines[i]}"
done
